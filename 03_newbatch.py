...
import numpy as np
import pandas as pd
from dbldatagen import DataGenerator, FakerTextFactory, fakerText
...
class BankDataGen:
    ...
    def dataGen(
        ...
        fakerDataspec = (
            ...
            .withColumn(
                "uni_loan_balance", "float", minValue=0.01, maxValue=10000, random=True
            )
            ...
            .withColumn(
                "fraud_trx", "string", values=["0", "1"], weights=[9, 1], random=True
            )
        )
    ...
    def saveFileToCloud(self, df):
        ...
        df.write.format("csv").mode("overwrite").save(
            self.storage + "/bank_fraud_demo/" + self.username
        )
    ...
    def createOrAppend(self, df):
        ...
        try:
            df.writeTo("{0}.CC_TRX_{1}".format(self.dbname, self.username)).using(
                "iceberg"
            ).tableProperty("write.format.default", "parquet").append()
...
def main():
    ...
    spark.read.format("iceberg").load(
        "{0}.CC_TRX_{1}.snapshots".format(DBNAME, USERNAME)
    ).show()
